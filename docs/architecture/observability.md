...
The MCP server exposes Prometheus metrics at `/metrics`. All 55 active tools are automatically instrumented via the `_ok()` / `_err()` return helpers — no per-tool code changes needed.
