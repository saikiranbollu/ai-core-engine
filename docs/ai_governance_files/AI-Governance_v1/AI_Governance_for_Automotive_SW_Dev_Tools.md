...
**Version:** 1.0 (draft for iteration) • **Date:** 18 April 2026 • **Scope:** AI-as-development-tool (Copilot/GPT4IFX building MCAL code), *not* AI-in-vehicle

---

## 0. Executive Summary

You are building a generative-AI development platform (AI Core Engine + ~20 Domain Assistants) that produces artifacts — requirements, architecture, design, C code, test specs, test code, config, review comments — which then flow into **ASIL-rated MCAL / iLLD productive software for AURIX TC3xx / TC4xx**. The AI does not run in the vehicle; its *output* does.
...
2. **GitHub Copilot Enterprise ≠ ISO 26262-qualified tool.** Microsoft has ISO/IEC 42001 certification for M365 Copilot (and by extension the Copilot family including GitHub Copilot Enterprise), which gives you AIMS-level trust. But **42001 certification does not satisfy 26262-8 tool qualification** — those are orthogonal. For ASIL-D MCAL code generation, you will need either method 1c (validation) executed in your own process, or method 1b (evaluate the tool's dev process) with MS evidence + gap-fill, or method 1d (development per a safety standard) which MS has not claimed.

3. **Your risk matrix needs three refinements** that I'll detail in §4: (a) some DAs scored "Low" are actually Medium when rated against ISO 26262-8 TCL; (b) the matrix is missing risk parameters that EU AI Act Article 14 (human oversight) and Article 15 (robustness/cybersecurity) explicitly require; and (c) the matrix doesn't currently distinguish between Copilot-backed DAs (external cloud) and GPT4IFX-backed DAs (internal), which is the biggest data-governance fork in your architecture.
