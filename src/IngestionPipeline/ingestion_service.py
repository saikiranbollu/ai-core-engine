...
class IngestionService:
    ...
    def batch_ingest(self, lld_path: str, modules: Optional[List[str]] = None,
        ...
        for i, mod in enumerate(modules):
            try:
                ...
                per_module.append({"module": mod, "status": "completed", **r})
            except Exception as e:
                per_module.append({"module": mod, "status": "failed", "error": str(e)})
    ...
    def _parse_file(self, path: Path, ext: str) -> Optional[Dict]:
        ...
        for .c, .h, .pdf, .xlsx, .arxml, .puml, .rst formats.
        """
        if ext in (".c", ".h"):
            try:
            ...
            with self._neo4j.session(database=db) as session:
                ...
                if ptype in ("c_header", "c_source"):
                    for fn in self._iter_c_functions(parsed):
                        ...
                        nodes += 1
                        rels += self._write_call_relationships(
                            session,
                            op,
                            normalized_module,
                            normalized_workspace,
                            fn["name"],
                            fn.get("internal_calls", []),
                        )
                    ...
                    for dep in parsed.get("dependencies", []):
                        ...
                        if src and tgt:
                            session.run(
                                f"MATCH (a:{CANONICAL_FUNCTION_LABEL} {{name: $src, module: $module}}) "
                                f"MATCH (b:{CANONICAL_FUNCTION_LABEL} {{name: $tgt, module: $module}}) "
                                f"{op} (a)-[:DEPENDS_ON]->(b)",
                                src=src, tgt=tgt, module=normalized_module,
                            )
                            rels += 1
