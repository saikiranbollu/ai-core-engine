...
**Version**: 1.1.0 (supersedes v1.0.0 of 2026-04-18)
**Classification**: Internal — Infineon Technologies
**Owner**: Safety Manager + AI Governance Lead + Platform Team
**Last Updated**: 2026-04-20
**Applies to**: **Zone A (productive mcal) only** — ASIL-A through ASIL-D software development in the `mcal` workspace. Zones B (illd) and C (foss-bsp) use a lighter "AI Usage Statement" instead (see §2.1 below).
...
- **Related documents:** AICE_SYSTEM_CARD v2.1.0; AI_USAGE_POLICY v2.1.0; VDA AI-in-QM Yellow Volume Ch. 7; **AICE-GOV-009 FOSS_LICENSE_COMPLIANCE** (for Zone B/C license obligations)

### 2.1 Scope Zones — What This Plan Covers (NEW v1.1.0)

ISO 26262-8 Clause 11 applies only where an ASIL claim exists on the output. AICE supports three scope zones with different treatment:

| Zone | Output | ASIL claim | Clause 11 applies? | Instrument |
|---|---|---|---|---|
| **A. Productive MCAL** (workspace: `mcal`) | Customer-delivered MCAL C, ARXML, safety analysis | Up to ASIL-D | **Yes** | **Full STQP / TCER per this document** |
| **B. iLLD reference SW** (workspace: `illd`) | Reference driver code, headers, examples published under Infineon Free License | **None (QM)** | No | "AI Usage Statement" (§2.2) — lightweight |
| **C. FOSS BSP** (workspace: `foss-bsp`) | Zephyr / NuttX BSP glue, device tree, kconfig | None (integrator qualifies as SOUP) | No | "AI Usage Statement" (§2.2) |

**The rest of this plan (§3 onwards) applies only to Zone A.** For Zones B and C, the clause 11 ceremony is dropped; governance responsibility shifts to license compliance (AICE-GOV-009) and standard engineering review.

### 2.2 AI Usage Statement (Zone B / C) — lightweight substitute

Each Zone B or Zone C DA has a short **AI Usage Statement** (one page) documenting:

1. DA name, version, LLM backend, AICE version
2. Task scope (what the DA is used for)
3. Explicit declaration: "No ASIL claim on outputs. ISO 26262-8 Clause 11 not applicable."
4. Downstream verification in place: MISRA advisory + Bugfinder + compile + peer review (Zone B); FOSS-project-native checks (Zone C)
5. License compliance controls: reference to AICE-GOV-009
6. AI transparency markers applied (code header, git trailer) — confirmed
7. Re-evaluation trigger: "If any output is later claimed against an ASIL target by Infineon or a downstream integrator, this DA must be formally qualified per §3 before that claim is valid."

**The statement is a living document**, ~1 page, maintained in the DA's source repo. Not assessor-facing unless zone changes.

### 2.3 Boundary condition — when does a Zone B/C output become a Zone A output?

If an iLLD driver is lifted into a productive MCAL delivery (happens — customers sometimes promote iLLD to production), **the current TCER from Zone A for that DA type applies**. The iLLD version itself doesn't inherit qualification; the productive-path use triggers the qualification burden at the point of promotion. Document in the customer delivery's safety case.
...
| **1.1.0** | **2026-04-20** | **Safety Manager + AI Governance Lead** | **Scope narrowed to Zone A (mcal productive) only. §2.1 explicit zone scope note; §2.2 AI Usage Statement as lightweight substitute for Zone B/C; §2.3 boundary condition when Zone B output is lifted to Zone A** |
