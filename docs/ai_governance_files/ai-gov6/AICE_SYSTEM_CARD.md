...
## 0. Changes from v1.0.0 (2026-03-29) and v2.0.0 (2026-04-18)

### v2.2.0 (2026-04-20) delta
Closes **Ephemeral Sandbox governance blind spot** identified in v2.1 review. New §5.1c sandbox lifecycle stage; §7.2 failure modes extended with sandbox-specific failures; §6.2 review gate gets ASIL-D + sandbox-grounded override rule. All controls defined in new AICE-GOV-010 SANDBOX_AND_EPHEMERAL_DATA_GOVERNANCE.md.

### v2.1.0 (2026-04-20) delta
This revision formalizes **governance scope zones** — AICE supports three distinct delivery zones with proportional governance: `mcal` (ASIL-D productive), `illd` (reference SW, public), and `foss-bsp` (Zephyr/NuttX BSP). v2.0.0 implicitly treated all outputs under the ASIL-D governance stack; v2.1.0 differentiates proportionally. New §3.2a scope-zone matrix; §5.1 data sources clarified (published iLLD is public, not confidential); §6.2 review gate rules differentiated per zone; §10 alignment matrix updated.

### v2.0.0 (2026-04-18) delta (unchanged)
...
### 3.2a Scope Zones — Proportional Governance (NEW v2.1.0)

AICE supports three distinct delivery zones. Governance controls are proportional to the zone, not uniform across AICE:

| Scope Zone | Output | Workspace(s) | Highest ASIL | License | Governance stack applied |
|---|---|---|---|---|---|
| **Zone A: Productive MCAL (customer delivery)** | AUTOSAR MCAL C code, ARXML, safety analysis, customer deliverables | `mcal`, `mcal_customer_X` | **ASIL-D** | Customer-NDA / IFX-confidential | **Full stack** — all controls in this document apply |
| **Zone B: iLLD reference SW** | iLLD C code, headers, example apps, docs | `illd` | **QM (no ASIL claim by Infineon)** | Infineon Free License (public on GitHub) | **Reduced stack** — see §3.2a below |
| **Zone C: FOSS BSP (Zephyr / NuttX)** | Device tree, kconfig, BSP driver code, build system | `foss-bsp` (new) | QM (Infineon ships as-is; integrator qualifies as SOUP) | Apache-2.0 (Zephyr) / BSD-3-Clause (NuttX) | **FOSS-specific stack** — see AICE-GOV-009 |
...
| EU AI Act Art. 53 (GPAI if applicable) | ✅ | ✅ | ✅ |
| ISO/IEC 42001 (AIMS) | ✅ | ✅ | ✅ |
| ISO 26262-8 Clause 11 (tool qualification) | ✅ (TCL1/TCL2 per AICE-GOV-007) | ❌ (no ASIL; "AI Usage Statement" only) | ❌ (no ASIL; integrator burden) |
| ISO/SAE 21434 (cybersecurity) | ✅ | ⚠️ (good practice, not formal) | ⚠️ (good practice, not formal) |
| ASPICE 4.0 SWE / SUP | ✅ | ⚠️ (work products internal, not capability-assessed) | ⚠️ (not assessed) |
| MISRA + AUTOSAR + Polyspace Bugfinder + CodeProver + MC/DC | ✅ (all mandatory) | ⚠️ (MISRA advisory + Bugfinder only) | ⚠️ (per FOSS project standard) |
| Static analysis on AI-generated code | ✅ | ✅ (lighter) | ✅ (project-native tools) |
| Review Gate (AUTO/QUICK/FULL) | FULL + independent + Safety Mgr for ASIL-D | AUTO permitted; QUICK default; no FULL mandatory | QUICK default; FULL for FFI-critical BSP areas (MPU, interrupt vectors, startup) |
| Data routing | GPT4IFX only | Copilot Enterprise permitted | Copilot Enterprise permitted |
| **FOSS license compliance scan** (NEW) | ⚠️ (recommended; controls usually sufficient) | ✅ (mandatory — see AICE-GOV-009) | ✅ (**mandatory** — upstream license rules vary: Apache-2.0 for Zephyr, BSD-3 for NuttX) |
| **Copilot "Block matching public code"** | N/A (Copilot not routed for mcal) | ✅ (**enforced**) | ✅ (**enforced**) |
| Human author-of-record | ✅ | ✅ | ✅ |
| PII scrubber | ✅ | ✅ | ✅ |
| Provenance chain | ✅ (complete) | ✅ (complete) | ✅ (complete, includes FOSS upstream info) |
| WORM audit | ✅ | ⚠️ (standard audit log; WORM optional) | ⚠️ (standard audit log) |
| Art. 73 serious incident reporting | ✅ (direct path via customer-delivered code) | ⚠️ (indirect — if downstream integrator claims iLLD in their safety case) | ⚠️ (indirect) |
| Export control / sanctions compliance | ✅ | ✅ (public release requires export clearance) | ✅ |

**Key principle:** Governance should be *proportional* — not every control from Zone A needs to apply to Zone B or C, but some controls (license scanning, transparency markers) are actually *more* important in Zone B/C due to public visibility and upstream contribution dynamics.
...
|--------|------|-------------|-----------------|
| iLLD **pre-release** source (internal branches, TC4xx unreleased) | C/H files | Infineon confidential | Tree-sitter AST parsing |
| iLLD **published release** source (on `github.com/Infineon`) | C/H files | **Public (Infineon Free License)** | Tree-sitter AST parsing |
| MCAL requirements | Jama Connect exports | Infineon confidential / Customer NDA | Jama XML/JSON parsers |
...
| **Zephyr upstream** (for ZA DA) | C/H, device tree, kconfig | **Public (Apache-2.0)** | Tree-sitter + devicetree parser |
| **NuttX upstream** (for NXA DA) | C/H, board config, kconfig | **Public (BSD-3-Clause)** | Tree-sitter + config parser |
...
**Zone-based routing reminder:** Published iLLD and FOSS BSP content being public does NOT change AI-origin marking, license-compliance, or transparency obligations — those get stricter for public-release work, not looser. See AICE-GOV-009.

### 5.1c Ephemeral Sandbox — Governance Scope (NEW v2.2.0)

The Ephemeral Sandbox is a per-session scratch workspace accepting arbitrary user uploads (HW PDFs, customer specs, meeting notes, compiler logs, ad-hoc files) for session-bounded retrieval augmentation. Content expires at session TTL.

**This is a distinct governance zone** — orthogonal to workspace zones A/B/C:

| Sandbox governance concern | Control | Reference |
|---|---|---|
| Content snapshot for reproducibility | WORM-backed content-addressable snapshots at generation time | AICE-GOV-010 §C1 |
| Provenance chain with sandbox references | `provenance.sandbox_docs[]` schema extension | AICE-GOV-010 §C2 |
| Data-class inheritance (session auto-upgrades) | Deterministic classifier on upload; Cerbos re-evaluation | AICE-GOV-010 §C3 |
| Prompt-injection defense (content-type guard) | Applied to sandbox path | AICE-GOV-010 §C4 |
| PII scrubber on sandbox-derived chunks | Applied before prompt assembly | AICE-GOV-010 §C5 |
| Session isolation (Redis + per-session Qdrant) | Enforced; no cross-session cache sharing | AICE-GOV-010 §C6 |
| Pattern store firewall (no patterns from sandbox-grounded generation) | Enforced by FeedbackSink pipeline | AICE-GOV-010 §C7 |
| Full audit trail (upload / reference / expire / promote) | audit_logs extended | AICE-GOV-010 §C8 |

**Zone A ASIL-D + sandbox rule:** Sandbox-grounded generation of ASIL-D artifacts forces FULL + independent + safety manager review regardless of confidence score. Reviewer must verify sandbox content against authoritative source; verification recorded in `review_evidence.sandbox_source_verified`.

**Zone C upstream rule:** Generations grounded on non-public sandbox content cannot be contributed upstream to Zephyr/NuttX. See AICE-GOV-009 §5.3 checklist extension.

### 5.2 Data Lifecycle

| Phase | Controls |
|-------|----------|
| **Ingestion (permanent KG)** | Admin-tier authorization required; `ingestion_jobs` table tracks source, parser, timestamp, node/relationship counts; signed ingestion bundles (manifest SHA256) for production |
| **Sandbox ingestion (per-session)** | User-tier upload via DA session; deterministic data-class classifier; content-type guard + PII scrubber at upload time; audit logged (§C8) |
| **Storage** | Dual workspace isolation (illd/mcal) with separate Neo4j databases per workspace; per-module NodeSet anchors prevent cross-module data bleed; per-customer sub-workspace for customer-NDA content; **sandbox content in per-session Qdrant collection + Redis session state — never in permanent stores** |
| **Access** | Cerbos RBAC; API keys mapped to principals; tool-tier enforcement; no cross-workspace queries; **sandbox access scoped to originating session only** |
| **Retention** | KG data persists until explicit re-ingestion; **audit logs retained ≥ product lifetime + 10 years** (automotive warranty convention, exceeds EU AI Act Art. 12 6-month floor); Ephemeral Sandbox session state expires with session TTL (default 1h); **sandbox WORM snapshots retained per AICE-GOV-010 §C1** (Zone A: product lifetime + 10y; Zone B/C: 3y) |
| **Deletion** | Module-level data can be cleared via admin tools; no personal data processed; deletion creates audit record; **sandbox expiration does NOT delete WORM snapshots** (for provenance reproducibility) |
...
### 6.2 Review Gate Enforcement (REVISED — v2.0.0 + v2.1.0)
...
**Zone B (illd — reference SW, no ASIL claim):**

| Review level | When |
|---|---|
| AUTO permitted | Default for non-critical additions (examples, docs, trivial code) |
| QUICK default | Most iLLD driver code, header changes |
| FULL (recommended, not mandatory) | Reference-SW areas with high downstream reuse (crypto, secure boot examples, MPU setup examples) |

**Zone C (foss-bsp — Zephyr / NuttX BSP):**

| Review level | When |
|---|---|
| QUICK default | Most BSP code (driver glue, simple device tree additions) |
| FULL | **FFI-critical BSP areas** — MPU config, interrupt vectors, startup code, hypervisor config, secure boot. These fail-silently on mistakes and are hard to catch downstream |
| Dual review (BSP + OS integrator) | Any change touching scheduler or memory protection |

**Governance rule (v2.0.0 technical enforcement):** The `is_safety_critical` signal must be set to `true` for any module with ASIL-B or above, and the `asil=D` metadata tag forces the review gate to FULL + independent + safety-manager. This is **implemented in `evaluate_confidence` tool logic** (Zone A only). Zones B and C use softer defaults per the tables above.
...
| **Sandbox WORM storage unavailable (NEW)** | Health check on WORM provider | **Fail-closed** for generation — sandbox-grounded generation blocked; user can proceed without sandbox or wait for recovery |
| **Sandbox content-type guard detects prompt-injection pattern** | Content-type scanner verdict | Upload rejected; reason returned to user; logged as LOW governance incident |
| **Sandbox data-class classifier ambiguous** | Classifier confidence below threshold | Upload accepted at most-restrictive candidate class; flagged for Platform Team review |
| **Sandbox Qdrant per-session collection cleanup fails** | TTL job failure | Flagged to Platform Team; collection retained for investigation; no cross-session exposure because collection names are session-scoped |
| **Cross-session semantic cache leak attempt (sandbox-derived entry retrieved by different session)** | `cache_shareable=false` flag check | Cache miss forced; logged; if repeated → indicates flag implementation bug |
| **Pattern store firewall refusal (C7 trigger)** | Pattern creation blocked for sandbox-grounded generation | Logged at LOW severity; expected behavior; no user action |
...
| 2.0.0 | 2026-04-18 | ATV MC D SW VDF | Major revision reflecting: GPT4IFX on-prem hosting → GPAI provider posture; ASIL-D confirmed as target; VDA AI-in-QM adoption; ISO/PAS 8800 explicit non-applicability; MLE de-scoped; China regs de-scoped; tool qualification path; PII scrubber; field-failure traceback; Art. 53 + Art. 73 obligations added |
| 2.1.0 | 2026-04-20 | ATV MC D SW VDF | Scope zones formalized: Zone A (mcal/ASIL-D) / Zone B (illd/reference) / Zone C (foss-bsp). Data source sensitivity clarified — published iLLD is public. Review Gate differentiated per zone. FOSS license compliance obligation added with reference to new AICE-GOV-009 |
| **2.2.0** | **2026-04-20** | **ATV MC D SW VDF** | **Ephemeral Sandbox governance gap closed: §5.1c new sandbox lifecycle stage; §5.2 data lifecycle updated with sandbox stage; §7.2 sandbox failure modes added. All sandbox controls defined in new AICE-GOV-010** |
