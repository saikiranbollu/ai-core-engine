...
Current: **v3.0.0** (effective 2026-05-02). Supersedes all prior versions. See §11 Document Control for material changes in v3.0.0.
...
- **GPAI-related incidents** — Copilot Enterprise OR GPT4IFX model drift, systemic behavior change, contractual changes affecting §4.3 preconditions
...
5. **Sandbox recovery (NEW v2.1.0):** If `provenance.sandbox_docs[]` is non-empty:
   - Retrieve each referenced sandbox snapshot from WORM storage by SHA-256 (per AICE-GOV-010 §C1)
   - Reconstruct the exact sandbox content fed to the LLM at generation time
   - Verify snapshot integrity (hash match, WORM write timestamp consistent with session)
   - Flag any snapshot that cannot be recovered — this is itself a CRITICAL finding (reproducibility failure; EU AI Act Art. 12 non-compliance)
   - Compare sandbox content against authoritative source at that time (e.g., Jama PRQ version, HW errata revision) — did the sandbox content itself contain an error that propagated into generation?
6. **Preliminary assessment (within 4h):** Is the failure plausibly caused by AI-generated content, including consideration of sandbox-grounded content?
7. **If yes:** escalate to CRITICAL per §3 → Phase 1
8. **If no:** continue standard field-incident process but document AI-non-causation
...
| **GPAI Model Drift — Copilot Enterprise (NEW v3.0.0)** | Microsoft Copilot Enterprise model behavior changed (silent backend rotation, version upgrade) without our notice; outputs degraded | Generation pattern shift coincident with Copilot version bump; regression benchmark failed; Microsoft service announcement correlation |
| **GPAI Model Drift — GPT4IFX** | GPT4IFX behavior changed between sessions without notice | Model version changed (specialized paths); regression benchmark failed |
...
| **Sandbox Content Error (NEW v2.1.0)** | Sandbox-uploaded content contained an error (wrong register bitfield, wrong timing, outdated errata) that propagated into AI-generated output | `provenance.sandbox_docs[*]` recovered from WORM; comparison against authoritative source reveals discrepancy |
| **Sandbox Content Loss (NEW v2.1.0)** | Sandbox snapshot not recoverable from WORM (integrity failure or snapshot never written); generation cannot be reproduced | Phase 0 step 5 fails: SHA mismatch, WORM retrieval error, or provenance.sandbox_docs references a snapshot_ref that returns 404. **This is itself a CRITICAL compliance failure** — EU AI Act Art. 12 reproducibility broken |
| **Sandbox Prompt Injection Leak (NEW v2.1.0)** | Content-type guard on sandbox path missed an injection pattern; adversarial instructions in an ingested PDF influenced LLM behavior | Recovered sandbox content contains hidden instructions; generated output aligns with injection intent, not with legitimate prompt |
| **Sandbox Data-Class Leak (NEW v2.1.0)** | Customer-NDA or Infineon-confidential content landed in a session routed to Copilot due to classifier miss or race condition | audit_logs shows Copilot call in session with sandbox content classified later as confidential; session upgrade did not fire before LLM call |
| **Semantic Cache Sandbox Bleed (NEW v2.1.0)** | Generation drew context from a cached entry whose original generation was sandbox-grounded, bypassing C7 pattern firewall | Cache hit records trace to an original generation with `cache_shareable=false` not honored; indicates cache flag bug |
...
| **GPAI Model Drift — Copilot Enterprise** (NEW v3.0.0) | Confirm Microsoft Copilot version change via Trust Center / Service Health; pin to known-good version where pin available; re-run regression benchmark; document in AIBOM; Legal monitors any §4.3 contractual implications | AI Governance Lead + Legal + Platform Team |
| **GPAI Model Drift — GPT4IFX** | Coordinate with GPT4IFX platform team; pin model version; re-run regression benchmarks; document in AIBOM | AI Governance Lead + GPT4IFX Platform Lead |
...
| **Sandbox Content Error** (NEW v2.1.0) | Customer notified of sandbox source revision needed (if their input was wrong) or authoritative-source process strengthened; upload checklist updated | Module Lead + relevant source owner |
| **Sandbox Content Loss** (NEW v2.1.0) | **Emergency**: audit WORM provider integrity; restore from backup if possible; if not recoverable, document gap in incident; **escalate as Art. 12 compliance incident** to Legal + AI Governance Lead; strengthen snapshot-write path (idempotency, retry, verification) | Platform Team + Legal |
| **Sandbox Prompt Injection Leak** (NEW v2.1.0) | Tighten content-type guard patterns; add detected payload to guard signature database; re-scan recent sandbox uploads for similar patterns; consider adversarial training data for guard | Platform Team + Security |
| **Sandbox Data-Class Leak** (NEW v2.1.0) | Review classifier logic; fix race condition if any; audit all sessions from past 30 days for similar leak; customer notification if NDA content reached Copilot; Legal involvement for contract implications | AI Governance Lead + Legal + Platform Team |
| **Semantic Cache Sandbox Bleed** (NEW v2.1.0) | Fix cache flag implementation; invalidate affected cache entries; audit all cached entries for flag correctness | Platform Team |
...
- **GPAI Model Drift root cause split** into Copilot Enterprise variant and GPT4IFX variant (with separate corrective-action paths)
- §2 GPAI scope explicitly covers both LLMs and Copilot §4.3 contractual changes
- Sandbox-related root causes (content error, content loss, prompt injection leak, data-class leak, semantic cache bleed) and Phase 0 sandbox recovery step preserved from prior version
- Legacy multi-version changelog removed
...
| Customer Interface Lead | __________ | __________ |
