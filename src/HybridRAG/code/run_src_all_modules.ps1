...
<#
.SYNOPSIS
    Re-ingest source code KG for ALL 30 modules.

.DESCRIPTION
    Runs step 6 (Source Code → Neo4j) of the main pipeline for all modules
    to apply the latest fixes (dispatcher-aware BFS, branch-conditional
    filtering, write-dominates Rule 2, callee propagation dedup).

    The builder automatically:
      - Deletes SRC_CALLS edges before re-creating
      - Deletes SRC_USES_GLOBAL edges before re-creating
      - Deletes SRC_ACCESSES_SFR edges before re-creating
    No manual stale-edge cleanup is needed.
...
.EXAMPLE
    .\run_src_all_modules.ps1 -Force
    .\run_src_all_modules.ps1 -Force -AutoFetch
    .\run_src_all_modules.ps1 -DryRun
#>
param(
...
Write-Host ""
Write-Host ("=" * 64)
Write-Host "  Source Code KG Ingestion — ALL Modules (Fresh Re-run)"
