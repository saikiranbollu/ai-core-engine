...
> **v3.0.0 context (System Card):** Copilot Enterprise is now the primary LLM across all zones. Microsoft is the GPAI provider for the Copilot path (ISO/IEC 42001 certified). **The Art. 53 obligation analysis in this document therefore applies only to GPT4IFX-served paths** (PDF extraction, RLM planning, fallback, contractually-precluded routing). Infineon's GPAI provider burden is significantly reduced — but not zero.
